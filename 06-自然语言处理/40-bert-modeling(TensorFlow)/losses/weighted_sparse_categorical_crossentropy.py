...
# 将标签处理成 batch,seq_len 的形状
def _adjust_labels(labels, predictions):
    """Adjust the 'labels' tensor by squeezing it if needed."""
    labels = tf.cast(labels, tf.int32)
    if len(predictions.shape) == len(labels.shape): # batch, seq_len, num_classes
        labels = tf.squeeze(labels, [-1])           # batch, seq_len
